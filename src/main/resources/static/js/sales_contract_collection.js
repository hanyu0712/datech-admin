...
$(document).ready(function() {
    ...
    $("#contractNo").change(function(){
        ...
        for (let i = 0; i < contract.length; i++) {
            if (contract[i].contractNo === selectVal){
                $("#customerName").val(contract[i].customerName);
                $("#projectName").val(contract[i].projectName);
                $("#salesPerson").val(contract[i].salesPerson);
    ...
    $('#salesContractModal').on('show.bs.modal', function (event) {
        ...
        if ("insert" === method) {
            ...
            $.ajax({
                url: document.getElementById("serverUrl").value +'/salesCollection/'+id,
                type: 'get',
                dataType: "json",
                success: function (data) {
                    ...
                    modal.find('#collectionDate').val(data.collectionDateStr);
                    modal.find('#collectionAmount').val(data.collectionAmount);
                    modal.find('#collectionBank').val(data.collectionBank);
                    modal.find('#collectionMode').val(data.collectionMode);
                    modal.find('#collectionCompany').val(data.collectionCompany);
    ...
    $(document).on('click', '#button-submit', function () {
        ...
        if (!contract_data.collectionDate) {
            $.gritter.add({
                title: '系统消息',
                text: "请选择交易日期"
            });
        ...
        if (!(!isNaN(parseFloat(contract_data.collectionAmount)) && isFinite(contract_data.collectionAmount))) {
            $.gritter.add({
                title: '系统消息',
                text: "金额只能输入数字"
            });
    ...
    $('#salesContractModal').on('hide.bs.modal', function () {
        ...
        modal.find('#collectionDate').val("");
        modal.find('#collectionMonth').val("");
        modal.find('#collectionAmount').val("");
        modal.find('#collectionBank').val("");
        modal.find('#collectionMode').val("");
        modal.find('#collectionCompany').val("");
        modal.find('#rejectionContent').val("");
...
{
    $.ajax({
        url: document.getElementById("serverUrl").value +'/updateSalesCollection',
        type: 'post',
        dataType: "json",
        data: contract_data,
        success: function (data) {
