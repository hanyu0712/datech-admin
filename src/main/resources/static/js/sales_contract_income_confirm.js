...
$(document).ready(function() {
    let contract;
    $.ajax({
        url: document.getElementById("serverUrl").value +'/salesInvoiceSelect',
        type: 'get',
        dataType: "json",
    ...
    $("#noTaxAmount").change(function(){
        let taxAmount = $("#taxAmount").val();
        let noTaxAmount = $("#noTaxAmount").val();
        $("#amountTotal").val(parseFloat(taxAmount) + parseFloat(noTaxAmount));
    });
    ...
    $('#salesContractModal').on('show.bs.modal', function (event) {
        ...
        if ("insert" === method) {
            ...
            $.ajax({
                url: document.getElementById("serverUrl").value +'/salesIncomeConfirm/'+id,
                type: 'get',
                dataType: "json",
                success: function (data) {
                    ...
                    modal.find('#invoiceDate').val(data.invoiceDateStr);
                    modal.find('#invoiceCode').val(data.invoiceCode);
                    modal.find('#invoiceNo').val(data.invoiceNo);
                    modal.find('#companyName').val(data.companyName);
                    modal.find('#noTaxAmount').val(data.noTaxAmount);
                    modal.find('#taxAmount').val(data.taxAmount);
                    modal.find('#amountTotal').val(data.taxAmount + data.noTaxAmount);
                    modal.find('#content').val(data.content);
                    modal.find('#managerName').val(data.managerName);
    ...
    $(document).on('click', '#button-submit', function () {
        ...
        if (!(!isNaN(parseFloat(contract_data.noTaxAmount)) && isFinite(contract_data.noTaxAmount))) {
            $.gritter.add({
                title: '系统消息',
                text: "不含税价只能输入数字"
            });
        ...
        if (!(!isNaN(parseFloat(contract_data.taxAmount)) && isFinite(contract_data.taxAmount))) {
            $.gritter.add({
                title: '系统消息',
                text: "税额只能输入数字"
            });
    ...
    $('#salesContractModal').on('hide.bs.modal', function () {
        ...
        modal.find('#invoiceDate').val("");
        modal.find('#invoiceCode').val("");
        modal.find('#invoiceNo').val("");
        modal.find('#companyName').val("");
        modal.find('#noTaxAmount').val("");
        modal.find('#taxAmount').val("");
        modal.find('#content').val("");
        modal.find('#managerName').val("");
        modal.find('#rejectionContent').val("");
...
{
    $.ajax({
        url: document.getElementById("serverUrl").value +'/updateSalesIncomeConfirm',
        type: 'post',
        dataType: "json",
        data: contract_data,
        success: function (data) {
